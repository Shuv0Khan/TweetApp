...
def verdict():
    with open('..\\data\\users_bio_twitter_roberta.csv', mode='r', encoding='utf-8') as fin, open(
            '..\\data\\users_bio_twitter_roberta_2.csv', mode='w', encoding='utf-8') as fout:
        lines = fin.readlines()
        fout.write(f'{lines[0].strip()},verdict\n')
        ...
        for i in range(1, len(lines)):
            ...
            for j in range(0, len(cols)):
                if max_prob < float(parts[j+1]):
                    max_prob = float(parts[j+1])
                    max_col = cols[j]

            fout.write(f'{l},{max_col}\n')
